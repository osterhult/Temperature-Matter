...
float getHumidity() { return measuredHumidity; }
float getTemperature() { return measuredTemperature; }
...
int readDHT()
{
	...
	printf("Send start signal to DHT sensor on pin: %d \n", DHTgpio);
	...
	uSec = getSignalLevel(85, 1); // Increase the timeout value to 200us for ESP32-H2
	//	ESP_LOGI( TAG, "Response = %d", uSec );
	if (uSec < 0)
	{
		printf("Timeout 2\n");
	...
	measuredHumidity = dhtData[0];
	measuredHumidity *= 0x100; // >> 8
	measuredHumidity += dhtData[1];
	measuredHumidity /= 10; // get the decimal

	// == get temp from Data[2] and Data[3]

	measuredTemperature = dhtData[2] & 0x7F;
	measuredTemperature *= 0x100; // >> 8
	measuredTemperature += dhtData[3];
